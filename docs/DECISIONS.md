...
| **Consequences** | This decision creates a mobile-only exception to DEC-004’s rule that deferred category labels stay hidden. DEC-004 remains binding for desktop activation. DEC-008’s dead-link prohibition remains fully binding: unavailable entries are non-link status rows; only real pages, anchors, and mail actions are navigable. Desktop navigation remains Platform-only until separately approved. |
| **Owner** | Product / Fatih |
